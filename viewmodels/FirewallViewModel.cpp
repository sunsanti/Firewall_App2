...
// //This is where we check packet state
// FirewallViewModel::ConnState
// FirewallViewModel::getPacketState(const unsigned char* pkt) {

//     if (PacketParser::isTCP_RST(pkt)) {
//         return FirewallViewModel::RELATED;
//     }

//     if (PacketParser::isIPv4(pkt) || PacketParser::isIPv6(pkt)) {
//         int tcp_flags = PacketParser::getTCPFlags(pkt);

//         if ((tcp_flags & TH_SYN) && !(tcp_flags & TH_ACK)) {
//             return FirewallViewModel::NEW;
//         }
//         return FirewallViewModel::ESTABLISHED;
//     }
...
//     QSqlQuery query(db);
//     if(!query.exec("SELECT ip, port, protocol, action, ip_version FROM firewall_rules")) 
//         return "ALLOW";
...
//         int protocol = query.value(2).toInt();
//         QString action = query.value(3).toString();
//         int ip_version = query.value(4).toInt();
...
//         bool portMatch = (packet.srcPort() == port);

//         // bool protocolMatch = (protocol == 0 || packet.protocol() == protocol);
...
//         bool portMatch = (packet.desPort() == port);
...
//     ConnState state = getPacketState(pktData);
//     const char* stateStr =
//     state == NEW ? "NEW" :
//     state == ESTABLISHED ? "ESTABLISHED" :
//     state == RELATED ? "RELATED" : "UNKNOWN";

//     if (state == ESTABLISHED || state == RELATED) {
//         return nfq_set_verdict(qh, id, NF_ACCEPT, 0, nullptr);
//     }

//     // ===== 2. CHỈ XỬ LÝ RULE CHO NEW =====
//     if (state != NEW) {
//         return nfq_set_verdict(qh, id, NF_ACCEPT, 0, nullptr);
//     }
...
//     if(result == "DENY") {
//         cout << (ipv6 ? "[IN][IPv6]2 " : "[IN][IPv4]2 ") << "DROP " << srcIP.toStdString() << ":" << srcPort << " Protocol=" << static_cast<int>(protocol) << "\n";
//         cout << "[VERDICT] DROP\n";
//         return nfq_set_verdict(qh, id, NF_DROP, 0, nullptr);
//     } else {
//         cout << (ipv6 ? "[IN][IPv6]2 " : "[IN][IPv4]2 ") << "ACCEPT " << srcIP.toStdString() << ":" << srcPort << " Protocol=" << static_cast<int>(protocol) << "\n";
//         cout << "[VERDICT] ACCEPT\n";
//         return nfq_set_verdict(qh, id, NF_ACCEPT, 0, nullptr);
//     }
//     return nfq_set_verdict(qh, id, NF_ACCEPT, 0, nullptr);
...
// cout << "[RULE RESULT] "
//      << result.toStdString()
//      << " dst=" << dstIP.toStdString()
//      << ":" << dstPort
//      << "\n";

// if(result == "DENY") {

//     cout << (ipv6 ? "[OUT][IPv6]2 " : "[OUT][IPv4]2 ")
//          << "REJECT " << dstIP.toStdString() << ":" << dstPort
//          << " Protocol=" << static_cast<int>(protocol) << "\n";

//     if(protocol == IPPROTO_TCP) {
//         cout << "[QUIC] TCP 443 detected\n";
//         if(ipv6) sendTcpRstIPv6(pktData);
//         else     sendTcpRst(pktData);
...
//         : srcIP(sIP), dstIP(dIP), srcPort(static_cast<quint16>(sPort)),
//           dstPort(static_cast<quint16>(dPort)), protocol(static_cast<quint8>(proto)) {}
//     bool operator==(const DnsTuple &other) const {
...
// QList<FirewallViewModel::DnsTuple> pendingTuples;

// void FirewallViewModel::addTuple(const DnsTuple &tuple) {
//     pendingTuples.append(tuple);
// }


// bool FirewallViewModel::matchAnyTuple(const QString &packetSrcIP, const QString &packetDstIP,
//                    int packetSrcPort, int packetDstPort, int protocol) {
//     for(const DnsTuple &t : pendingTuples) {
//         if(packetSrcIP == t.dstIP &&
//            packetDstIP == t.srcIP &&
//            packetSrcPort == t.dstPort &&
//            packetDstPort == t.srcPort &&
//            protocol == t.protocol) {
//             // Trùng tuple -> xóa khỏi pending
//             pendingTuples.removeOne(t);
//             return true;
//         }
//     }
//     return false;
...
QString FirewallViewModel::inferState(
    int protocol,
    ...
    if (protocol == IPPROTO_TCP) {
        ...
        if (!ipv6) {
            ...
            tcp = (struct tcphdr*)(pktData + ip->ihl * 4);
        } else {
            tcp = (struct tcphdr*)(pktData + 40); // IPv6 fixed header
        }
...
FirewallViewModel::FirewallViewModel(QObject* parent) : QObject(parent) {
    m_rulesCache = loadRules();
}
QSqlDatabase FirewallViewModel::dbForThread()
{
    QString connName =
        QString("nfqueue_conn_%1")
    ...
    db.setHostName("192.168.100.90");
    db.setPort(5432);
    db.setDatabaseName("firewall_db");
    db.setUserName("postgres");
    db.setPassword("123456");

    if (!db.open()) {
        qDebug() << "[NFQUEUE][DB] Open failed:" << db.lastError().text();
    }
...
bool FirewallViewModel::deleteRule(int id) {
    ...
    // --- Cập nhật cache ---
    QWriteLocker locker(&m_rulesLock);
    for (int i = 0; i < m_rulesCache.size(); ++i) {
        if (m_rulesCache[i].id() == id) {
            m_rulesCache.remove(i);
            break;
        }
    }
...
QVector<FirewallRule> FirewallViewModel::loadRules() {
    ...
    QSqlDatabase db = dbForThread();
    QSqlQuery query(db);
    query.exec("SELECT id, ip, port, protocol, action, ip_version FROM firewall_rules");
    while (query.next()) {
        int id = query.value(0).toInt();
        QString ip = query.value(1).toString();
        int port = query.value(2).toInt();
...
//this use to handle the packet from computer to computer
//This is to check the income and out go packet
QString FirewallViewModel::checkPacketIncoming(const FirewallPacket &packet, bool ipv6) {
    QReadLocker locker(&m_rulesLock);

    QString allowAction = "ALLOW";

    for (const FirewallRule& rule : m_rulesCache) {
        QString ip = rule.ip();
        int port = rule.port();
        QString action = rule.action();
        int ip_version = rule.ip_version();
...
QString FirewallViewModel::checkPacketOutgoing(const FirewallPacket &packet, bool ipv6) {
    QReadLocker locker(&m_rulesLock);
...
if (PacketParser::isTCP_RST(pktData)) {
    ...
    if(PacketParser::isIPv4(pktData)) {
        ...
        cout << "khong phai goi tin hop le";
        return nfq_set_verdict(qh, id, NF_ACCEPT, 0, nullptr);
    }

    FirewallPacket packet(srcIP, desIP, srcPort, desPort, protocol, "");
    QString result = checkPacketIncoming(packet, ipv6);

    cout << "DAY LA DE CHECK PACKET CO TAO RA CHUA\n";
    ...
    if(protocol == IPPROTO_TCP){
        ...
        // if(srcPort == 53){
        //     cout << "Day la cai noi ma RES ve ne\n";
        //     if(dnsManager.matchAndRemove(srcIP,desIP,srcPort,desPort,protocol)){
        //         cout << "Accept DNS request\n";
        //         return nfq_set_verdict(qh, id, NF_ACCEPT, 0, nullptr);
        //     } else {
        //         cout << "result: " << result.toStdString()  << endl;
        //         cout << "srcPort =" << srcPort << endl;
        //         cout << "Deny DNS request\n";
        //         return nfq_set_verdict(qh, id, NF_DROP, 0, nullptr);
        //     }
        // }
        if(result == "DENY") {
...
int FirewallViewModel::processOutgoing(struct nfq_data* nfa, struct nfq_q_handle* qh) {
    ...
    if(PacketParser::isIPv4(pktData)) {
        ...
        cout << "[OUT][IPv4] " << srcIP.toStdString() << ":" << srcPort << " → " << dstIP.toStdString() << ":" << dstPort
             << " IHL=" << PacketParser::getIPv4HeaderLen(pktData) << "\n";
    }
    else if(PacketParser::isIPv6(pktData)) {
        ipv6 = true;
...
// NEW → đưa vào engine xét rule
FirewallPacket packet(srcIP, dstIP, srcPort, dstPort, protocol, "");
QString result = checkPacketOutgoing(packet, ipv6);
    ...
    if(protocol == IPPROTO_TCP){
        if(result == "DENY") {
            ...
            cout << (ipv6 ? "[OUT][IPv6]2 " : "[OUT][IPv4]2 ")
                 << "REJECT " << dstIP.toStdString() << ":" << dstPort << " Protocol=" << static_cast<int>(protocol)  << "\n";
...
std::cout << "Packet processing time (us): " << duration << std::endl;

            cout << (ipv6 ? "[OUT][IPv6]2 " : "[OUT][IPv4]2 ") << "ACCEPT " << dstIP.toStdString() << ":" << dstPort << " Protocol=" << static_cast<int>(protocol) << "\n";
            return nfq_set_verdict(qh, id, NF_ACCEPT, 0, nullptr);
        }
    } else if(protocol == IPPROTO_UDP){
        // if(dstPort == 53){
        //     cout << "This is UDP sending request to DNS\n";
        //     ::DnsTuple dnsReq(srcIP, dstIP, srcPort, dstPort, protocol);
        //     dnsManager.addTuple(dnsReq);
        // }
        if(result == "DENY") {
